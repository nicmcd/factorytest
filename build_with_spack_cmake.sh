...
BRANCH=cmake
BUILD_SPEC="factorytest@${BRANCH} target=${SPACK_TARGET}"
...
spack spec ${BUILD_SPEC} > /tmp/factorytest_spec_${BRANCH}

# Extracts the exact spec of a dependency then returns the install directory
function location() {
    local spec=$(grep "\^${1}@" /tmp/factorytest_spec_${BRANCH} | sed 's/\s.*^//g')
